    ...
    with Workflow() as wf:
        ...
        if image_wan_teacache == "true":
            ...
            if "fun" in params.model:
                model = TeaCache(model=model, model_type='wan2.1_t2v_1.3B_ret_mode', rel_l1_thresh=0.05, end_percent=3)
            else:
                # Assume model is wan i2v 480p 14B
                model = TeaCache(model=model, model_type='wan2.1_i2v_480p_14B_ret_mode', rel_l1_thresh=0.3, end_percent=3)
...
async def _do_wan(params: ImageWorkflow, interaction):
    ...
    with Workflow() as wf:
        ...
        if t2v_wan_teacache == "true":
            model = TeaCache(model=model, model_type='wan2.1_t2v_1.3B_ret_mode', rel_l1_thresh=0.15, end_percent=3)
        model = ModelSamplingSD3(model, 8)
